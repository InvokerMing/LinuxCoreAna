...
/**
 * @brief 结构体 - 接收信号的进程端维护着的一个信号队列，该队列是一个sigqueue结构的链表。
 */
struct sigqueue {
	struct sigqueue *next;
	siginfo_t info; /* 信号所携带信息 */
};

/**
 * @brief 结构体 - sigqueue队列的列头。
 */
struct sigpending {
	struct sigqueue *head, **tail;
	/* 用其中各个位的值来表示对应信号的到来情况：1表示信号已经到来，但还未被进程处理；0表示该位对应的信号未到，或已处理完毕 */
	sigset_t signal;
};
